...
let lookupFunc name = try(Hashtbl.find funcTbl name) with
	| Not_found -> print_endline ("[Codegen] Cannot find label for procedure call " ^ name); 1

(* Walk through the lists again to replace Goto labels with their specific instruction line number *)
let remapLabel irc = List.map 
...
let codegen i (irc : Intermediate.irc) = match irc with
  | IRC(cmds) -> let res = List.fold_left
				( fun ls el -> match el with
					...
					| IRC_Assign (s1, IRC_And (x, y)) -> (fst ls)+1, (snd ls)@[Skip] (* And will never appear in VM, shown here for complete matching sake *)
					...
					| IRC_Call (s1, nos) -> let l1 = lookupFunc s1 in
