...
let codegen i (irc : Intermediate.irc) = match irc with
  | IRC(cmds) -> let res = List.fold_left
				( fun ls el -> match el with
					...
					| IRC_Assign (s1, IRC_Plus (x, y)) -> (fst ls)+4, (snd ls)@[PushToStack x;PushToStack y;Add;AssignFromStack (1, s1)]
					| IRC_Assign (s1, IRC_Minus(x, y)) -> (fst ls)+4, (snd ls)@[PushToStack x;PushToStack y;Sub;AssignFromStack (1, s1)]
					| IRC_Assign (s1, IRC_Times(x, y)) -> (fst ls)+4, (snd ls)@[PushToStack x;PushToStack y;Mult;AssignFromStack (1, s1)]
					| IRC_Assign (s1, IRC_Division(x,y)) -> (fst ls)+4, (snd ls)@[PushToStack x;PushToStack y;Div;AssignFromStack (1, s1)]
					| IRC_Assign (s1, IRC_Not (x)) -> (fst ls)+3, (snd ls)@[PushS 1;PushToStack x;Sub;AssignFromStack (1, s1)]
					| IRC_Assign (s1, IRC_Gt (x, y)) -> (fst ls)+4, (snd ls)@[PushToStack x;PushToStack y;Gt;AssignFromStack (1, s1)]
					| IRC_Assign (s1, IRC_Eq (x, y)) -> (fst ls)+4, (snd ls)@[PushToStack x;PushToStack y;Eq;AssignFromStack (1, s1)]
					| IRC_Assign (s1, IRC_And (x, y)) -> (fst ls)+1, (snd ls)@[Halt] (* And will never appear in VM, shown here for complete matching sake *)
					| IRC_Assign (s1, IRC_Var (x)) -> (fst ls)+2, (snd ls)@[PushToStack x;AssignFromStack (1, s1)]
					...
					| IRC_NonzeroJump (s1, l1) -> let lineNo = Hashtbl.find tbl l1 in
												  (fst ls)+3, (snd ls)@[PushToStack s1;NonZero lineNo]
